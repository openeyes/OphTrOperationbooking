...
<div class="row">
    ...
    ?></td>
									<td class="confirm"><input type="hidden" name="confirm_<?php echo $booking->element_id?>" value="0" /><input id="confirm_<?php echo $booking->element_id?>" type="checkbox" value="1" name="confirm_<?php echo $booking->element_id?>" disabled="disabled" <?php if ($booking->confirmed) {
    ?>checked="checked" <?php 
}
    ?>/></td>
    ...
    ?>
											<img src="<?php echo $assetPath?>/img/diaryIcons/warning.png" alt="<?php echo implode(' / ', $msgs);
    ?>" title="<?php echo implode(' / ', $msgs);
    ?>" width="17" height="17" />
										<?php 
}
    ?>
										<img src="<?php echo $assetPath?>/img/diaryIcons/confirmed.png" alt="confirmed" width="17" height="17" class="confirmed" title="confirmed"<?php if (!$booking->confirmed) {
    ...
    ?>
									...
									<span data-currproccount="<?php echo $session->getBookedProcedureCount() ?>" class="procedure-count" id="procedure_count_<?php echo $session->id ?>" <?php if (!$session->max_procedures) {
    ?>style="display: none;"<?php 
}?>><br />
    ...
    echo "0";
										...
										<span class="overbooked"<?php if ($session->getAvailableProcedureCount() >= 0) {
    ?> style="display: none;"<?php 
}?>>
										(Overbooked by <span class="overbooked-proc-val"><?= abs($session->getAvailableProcedureCount()); ?></span>)</span>
									</span>
									<span class="session-unavailable" id="session_unavailable_<?php echo $session->id?>" <?php if ($session->available) {
    ?>style="display:none;" <?php 
}?>> - session unavailable
										<span id="session_unavailablereason_<?php echo $session->id ?>"><?php if ($session->unavailablereason) {
    echo " - " . $session->unavailablereason->name;
} ?></span>
									</span>
								</div>
								<div class="specialists<?php if (!$session->consultant && !$session->anaesthetist && !$session->paediatric && !$session->max_procedures) {
    echo " hidden";
}?>">
									<div<?php if (!$session->consultant) {
    ?> style="display: none;"<?php 
}?> id="consultant_icon_<?php echo $session->id?>" class="consultant" title="Consultant Present">Consultant</div>
									<div<?php if (!$session->anaesthetist) {
    ?> style="display: none;"<?php 
}?> id="anaesthetist_icon_<?php echo $session->id?>" class="anaesthetist" title="Anaesthetist Present">Anaesthetist<?php if ($session->general_anaesthetic) {
    ?> (GA)<?php 
}?></div>
									<div<?php if (!$session->paediatric) {
    ?> style="display: none;"<?php 
}?> id="paediatric_icon_<?php echo $session->id?>" class="paediatric" title="Paediatric Session">Paediatric</div>
									<div<?php if (!$session->max_procedures) {
    ?> style="display: none;"<?php 
}?> id="max_procedures_icon_<?php echo $session->id?>" class="max-procedures" title="Max <?php echo $session->max_procedures ?>">Max <span class="max-procedures-val"><?php echo $session->max_procedures ?></span> Procedures</div>
								</div>
							</td>
						</tr>
						</tfoot>
    ...
    ?>
						<div class="panel session-options hidden diaryEditMode" data-id="<?php echo $session->id?>">
							...
							<label>
								<input type="checkbox" id="consultant_<?php echo $session->id?>" name="consultant_<?php echo $session->id?>" value="1"<?php if ($session->consultant) {
    ?> checked="checked"<?php 
}
    ?> />
								Consultant present
							...
							<label>
								<input type="checkbox" id="paediatric_<?php echo $session->id?>" name="paediatric_<?php echo $session->id?>" value="1"<?php if ($session->paediatric) {
    ?> checked="checked"<?php 
}
    ?> />
								Paediatric
							...
							<label>
								<input type="checkbox" id="anaesthetist_<?php echo $session->id?>" name="anaesthetist_<?php echo $session->id?>" value="1"<?php if ($session->anaesthetist) {
    ?> checked="checked"<?php 
}
    ?> />
							...
							<label>
								<input type="checkbox" id="general_anaesthetic_<?php echo $session->id?>" name="general_anaesthetic_<?php echo $session->id?>" value="1"<?php if ($session->general_anaesthetic) {
    ?> checked="checked"<?php 
}
    ?> />
							...
							<label>
								<input type="checkbox" class="session-available" id="available_<?php echo $session->id?>" name="available_<?php echo $session->id?>" value="1"<?php if ($session->available) {
    ?> checked="checked"<?php 
}
    ?> />
								Session available
