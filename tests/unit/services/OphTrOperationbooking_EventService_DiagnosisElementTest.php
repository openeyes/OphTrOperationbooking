...
class OphTrOperationbooking_EventService_DiagnosisElementTest extends \ModuleTestCase
{
	...
	public function testModelToResource()
	{
		...
		$this->compareDiagnosisElementResource($resource->elements[0], $event->elements[0]);
	}

	public function compareDiagnosisElementResource($resource, $model)
	{
		$this->assertInstanceOf('OEModule\OphTrOperationbooking\services\Element_OphTrOperationbooking_Diagnosis',$resource);
		$this->assertEquals($model->id,$resource->getId());

		$this->compareResourceFields($resource, $model, array('eye'));
		$this->compareResourceReferences($resource, $model, array('disorder'));
	}
	...
	public function testResourceToModel_NoSave_ModelIsCorrect()
	{
		...
		$this->compareDiagnosisElementModel($event->elements[0],$resource->elements[0]);
	}

	public function compareDiagnosisElementModel($model, $resource)
	{
		$this->assertInstanceOf('Element_OphTrOperationbooking_Diagnosis',$model);

		if ($resource->getId() !== null) {
			$this->assertEquals($resource->getId(),$model->id);
		}

		$this->compareModelFields($model, $resource, array('eye'));
		$this->compareModelReferences($model, $resource, array('disorder'));
	}
