...
class m140108_154140_default_arrival_time_for_sequences_and_sessions extends CDbMigration
{
	public function up()
	{
		...
		foreach (Yii::app()->db->createCommand()->select("distinct(start_time) as start_time")->from("ophtroperationbooking_operation_sequence")->queryAll() as $sequence) {
		...
		foreach (Yii::app()->db->createCommand()->select("distinct(start_time) as start_time")->from("ophtroperationbooking_operation_session")->queryAll() as $session) {
