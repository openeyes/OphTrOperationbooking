<?php /**
	...
	public function calculateEROD($booking_session_id) {
		...
		$lead_time_date = date('Y-m-d',strtotime($this->decision_date) + (86400 * 7 * Config::get('erod_lead_time_weeks')));
	...
	public function cancel($reason_id, $comment = null) {
		...
		if ($this->booking) {
			...
			if (Config::has('urgent_booking_notify_hours') && Config::has('urgent_booking_notify_email')) {
				if (strtotime($this->booking->session_date) <= (strtotime(date('Y-m-d')) + (Config::get('urgent_booking_notify_hours') * 3600))) {
					...
					foreach ($targets as $email) {
						mail(
							...
							"From: " . Config::get('urgent_booking_notify_email_from')."\r\n"
						);
					}
				}
			}
	...
	public function schedule($booking_attributes, $operation_comments, $session_comments, $reschedule=false) {
		...
		if (Config::has('urgent_booking_notify_hours') && Config::has('urgent_booking_notify_email')) {
			if (strtotime($session->date) <= (strtotime(date('Y-m-d')) + (Config::get('urgent_booking_notify_hours') * 3600))) {
				$targets = Config::get('urgent_booking_notify_email');
				if (!is_array($targets)) {
					$targets = array($targets);
				}
